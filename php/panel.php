...
// Notas propias
$resPropias = pg_query($conexion, "SELECT * FROM nota WHERE id_user = $id_user ORDER BY fecha_creado DESC;");
$notasPropias = [];
...
$resCompartidas = pg_query($conexion, "
...
if (isset($error)) {
    ...
    foreach ($notasPropias as $nota) {
        echo '<div class="nota">
            ...
            <p><strong>Etiquetas:</strong> ' . htmlspecialchars(implode(', ', $nota['etiquetas'])) . '</p>
...
if (count($notasCompartidas) > 0) {
    foreach ($notasCompartidas as $nota) {
        echo '<div class="nota">
            <p>' . nl2br(htmlspecialchars($nota['contenido'])) . '</p>
            <p><em>Autor: ' . htmlspecialchars($nota['autor']) . ' | Creada: ' . $nota['fecha_creado'] . '</em></p>
        </div>';
    }
} else {
